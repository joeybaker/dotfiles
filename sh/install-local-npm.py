...
def finished():
  ...
  print "If you want to uninstall, run:\n\n    launchctl unload ~/Library/LaunchAgents/com.joeybaker.localnpm.plist\n"

install_dir = os.path.abspath('/Volumes/jbdb/local-npm')
...
<plist version="1.0">
  <dict>
    <key>Label</key>
    <string>com.joeybaker.localnpm</string>
...
out_file = open(os.path.expanduser('~/Library/LaunchAgents/com.joeybaker.localnpm.plist'), 'w')
out_file.write(plist)
out_file.close()

command('launchctl load -w ~/Library/LaunchAgents/com.joeybaker.localnpm.plist')
