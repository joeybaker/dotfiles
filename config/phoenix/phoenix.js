...
function showHints(windows, prefix = "") {
  ...
  if (windows.length > HINT_CHARS.length) {
    var lists = Array.from(
      _.groupBy(windows, function (win, k) {
...
function buildhint(msg, win) {
  ...
  modal.origin = {
    ...
    y: Math.min(
      Math.max(
        Screen.all()[0].frame().height - (wf.y + wf.height / 2 + mf.height / 2),
        wsf.y
      ),
